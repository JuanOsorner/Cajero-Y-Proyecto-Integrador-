...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeProfilePage() {
        ...
        // Asegúrate de que la instancia sea de tipo Usuario (no Administrador)
        currentUserInstance = usuarios.find(u => u.id === loggedInUserData.id && u instanceof Usuario);

        if (!currentUserInstance) { // Si no se encontró el usuario o no es una instancia de Usuario (podría ser Admin)
            alert('Error: No se pudo cargar la información del usuario o no tienes permiso para acceder a este perfil. Vuelve a iniciar sesión.');
            sessionStorage.removeItem('loggedInUser'); // Limpiar la sesión rota si hay inconsistencia
    ...
    if (profileForm) { // Asegurarse de que el formulario existe en el DOM
        profileForm.addEventListener('submit', (e) => {
            ...
            if (!currentUserInstance) {
                profileStatusMessage.textContent = 'Error: No hay usuario logueado.';
            ...
            // Actualizar la instancia del usuario usando el método de la clase
            if (currentUserInstance.actualizarPerfil(nuevosDatos)) {
                ...
                // Actualizar también la información en sessionStorage para la cabecera (header_logic.js)
                // si el nombre o el correo cambiaron.
                sessionStorage.setItem('loggedInUser', JSON.stringify({
                    id: currentUserInstance.id,
                    nombre: currentUserInstance.nombre,
                    correo: currentUserInstance.correo,
                    role: currentUserInstance instanceof Administrador ? 'admin' : 'user' // Mantener el rol
                }));
